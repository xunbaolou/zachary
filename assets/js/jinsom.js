...
function jinsom_update_bbs_setting() {
    ...
    //判断必填项是否为空
    if (!showposts) {
        layer.msg('请输入帖子相关->帖子数量');
        return false;
    }
    if (showposts < 5) {
        layer.msg('帖子数量要大于或等于5');
        return false;
    }
    ...
    $.ajax({
        ...
        success: function(results) {
            ...
            clipboard.on('success', function(e) {
                ...
                function d() {
                    $('#jinsom-copy-share-link-single').children('g').remove()
                }
                setTimeout(d, 1000);
            });
...
function jinsom_pay_for_visible(post_id) {
    ...
    $.ajax({
        ...
        success: function(msg) {
            ...
            if (msg.code == 1) {
                function d() {
                    window.location.href = jinsom.home_url + '/?p=' + post_id;
                }
                setTimeout(d, 1500);
...
function jinsom_update_profile_bg_music() {
    ...
    $.ajax({
        ...
        data: {
            ...
            bg_music_on_off: bg_music_on_off
        },
        success: function(msg) {
            layer.closeAll('loading');
            layer.msg(msg.msg);
        ...
        success: function(msg) {
            ...
            layui.use(['upload', 'form', 'element'], function() {
                ...
                upload.render({
                    ...
                    accept: 'file',
                    data: {
                        author_id: $('#jinsom-bg-music-url').attr('data')
                    },
                    before: function(obj) {
...
function jinsom_post_link(obj) {
    ...
    function d() {
        $(obj).parents('a').attr('href', post_url);
    }
    setTimeout(d, 1500);
}
