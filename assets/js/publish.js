...
//添加文章背景图片功能开始
//弹出选择背景封面图的窗口
function jinsom_publish_bg_form(){
layer.load(1);
$.ajax({
type: "POST",
url:jinsom.jinsom_ajax_url+"/publish/select-bg.php",
success: function(msg){
layer.closeAll('loading');
a=layer.open({
type:1,
...
btn: false,
area: ['630px', '336px'],
skin: 'jinsom-publish-bg-form',
content: msg
})
$('.jinsom-publish-select-bg li').click(function(){
bg=$(this).attr('data');
$('#jinsom-pop-bg').val(bg);
$('#jinsom-pop-content').removeAttr('class').addClass('bg-'+bg);
layer.close(a); 
});	
}
});   
...
//文章弹出选择背景封面
function jinsom_publish_single_bg_form(){
layer.load(1);
$.ajax({
type: "POST",
url:jinsom.jinsom_ajax_url+"/publish/select-bg.php",
...
$('.jinsom-publish-select-bg li').click(function(){
bg=$(this).attr('data');
$('#jinsom-single-bg').val(bg);
$("#ueditor_1").contents().find("body").removeAttr('class').addClass('view bg-'+bg);
layer.close(a); 
});	
}
});   
