...
document.write("<script type='text/javascript' src='" + jinsom.cdn_url + "/mobile/js/edit.js'></script>"); //编辑相关
...
function jinsom_buy_post_form(post_id) {
    ...
    myApp.getCurrentView().router.load({
        url: jinsom.theme_url + '/mobile/templates/page/post-buy.php?post_id=' + post_id
    });
}
    ...
    $.ajax({
        ...
        success: function(msg) {
            ...
            if (msg.code == 1) {
                ...
                myApp.getCurrentView().router.load({
                    url: jinsom.theme_url + '/mobile/templates/page/mywallet/recharge-credit.php'
                });
            }
        }
    });
...
function jinsom_upload_avatar_menu(obj, user_id) {
    var buttons1 = [{
        ...
        },
        {
            ...
            onClick: function() {
                myApp.getCurrentView().router.load({
                    url: jinsom.theme_url + '/mobile/templates/page/upload-avatar.php?user_id=' + user_id
                });
            }
        }
    ];
...
function jinsom_send_gift_page(author_id) {
    ...
    myApp.getCurrentView().router.load({
        url: jinsom.theme_url + '/mobile/templates/page/send-gift.php?author_id=' + author_id
    });
}
...
function jinsom_reprint_form(post_id) {
    layer.closeAll();
    myApp.getCurrentView().router.load({
        url: jinsom.theme_url + '/mobile/templates/page/reprint.php?post_id=' + post_id
    });
}
...
function jinsom_get_redbag(post_id, obj) {
    myApp.getCurrentView().router.load({
        url: jinsom.theme_url + '/mobile/templates/page/get-redbag.php?post_id=' + post_id
    });
    if ($(obj).text() == '打开') {
...
function jinsom_task_treasure_form(task_id) {
    myApp.getCurrentView().router.load({
        url: jinsom.theme_url + '/mobile/templates/page/task-treasure.php?task_id=' + task_id
    });
}
...
function jinsom_pet_store(number) {
    myApp.getCurrentView().router.load({
        url: jinsom.theme_url + '/mobile/templates/page/pet-store.php?number=' + number
    });
}
...
function jinsom_lightbox() {
    $("[data-fancybox]").fancybox({
        ...
        btnTpl: {
            collect: '<a  onclick=\'jinsom_collect("","img",this)\' class="fancybox-button fancybox-button--collect" href="javascript:;"><i class="jinsom-icon jinsom-shoucang1"></i></a>'
        },
        mobile: {
        ...
        afterShow: function(instance, current) {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    url: $.fancybox.getInstance().current.src
                },
                success: function(msg) {
                    if (msg.code == 1) {
